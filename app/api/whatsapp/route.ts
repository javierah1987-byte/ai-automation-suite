...
export async function POST(req: NextRequest) {
  try {
    ...
    const res = await anthropic.messages.create({
      ...
      messages: [{ role: "user", content: `Eres el asistente virtual de WhatsApp del negocio "${businessName}".\nINFO: ${businessInfo}\nHISTORIAL: ${historyText}\nMENSAJE: ${message}\nResponde amable, breve (máx 3 frases), en español. NO inventes datos que no estén en la INFO. Si no sabes algo, di que te pondrás en contacto.` }]
    });
    const reply = res.content[0].type === "text" ? res.content[0].text : "Gracias por tu mensaje. En breve te atendemos.";
    return NextResponse.json({ reply });
  } catch (e: any) {
    return NextResponse.json({ reply: "Disculpa, error técnico momentáneo. Llámanos directamente." });
  }
}
